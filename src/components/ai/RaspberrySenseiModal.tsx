    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 lg:p-6 overflow-hidden">
        ...
        >
          ...
          {/* Modal Header */}
          <div className="relative z-10 px-3.5 sm:px-6 py-3 sm:py-4 border-b border-white/[0.08] bg-white/[0.02] backdrop-blur-md flex items-center justify-between">
            <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
              <div className="relative shrink-0">
                <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-2xl bg-gradient-to-tr from-rose-600 via-pink-500 to-amber-400 p-[1.5px] shadow-[0_4px_20px_rgba(244,63,94,0.4)] flex items-center justify-center">
                  <div className="w-full h-full bg-[#0d1222] rounded-[14px] flex items-center justify-center">
                    <RaspberryIcon size="sm" variant="rose" />
                  </div>
                </div>
                <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 sm:w-3.5 sm:h-3.5 bg-emerald-500 rounded-full border-2 border-[#0c101c] shadow-[0_0_8px_rgba(16,185,129,0.8)]" />
              </div>

              <div className="min-w-0">
                <div className="flex items-center gap-1.5 sm:gap-2">
                  <h3 className="text-sm sm:text-lg font-bold text-white tracking-tight flex items-center gap-1 truncate">
                    Малиновий Сенсей
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
              ...
              <button
                onClick={onClose}
                className="p-1.5 sm:p-2 rounded-xl text-slate-400 hover:text-white hover:bg-white/[0.08] transition-colors"
                title="Закрити вікно"
              >
                <X className="w-5 h-5" />
              </button>
          ...
          <div className="relative z-10 px-3.5 sm:px-6 py-2 sm:py-2.5 bg-white/[0.015] border-b border-white/[0.06] flex items-center gap-1.5 sm:gap-2 overflow-x-auto no-scrollbar">
            <span className="text-[10px] sm:text-[11px] font-semibold text-rose-400 whitespace-nowrap flex items-center gap-1 shrink-0">
            ...
            {PRESET_TOPICS.map((topic, index) => (
              <button
                ...
                disabled={isLoading}
                className="whitespace-nowrap text-[11px] sm:text-xs px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-full bg-white/[0.04] hover:bg-rose-500/20 text-slate-300 hover:text-rose-200 border border-white/[0.08] hover:border-rose-500/40 transition-all active:scale-95 disabled:opacity-50 flex items-center gap-1 sm:gap-1.5 flex-shrink-0"
              >
          ...
          <div className="flex-1 overflow-y-auto px-3.5 sm:px-6 py-4 sm:py-6 space-y-4 sm:space-y-6 relative z-10 custom-scrollbar">
            {messages.map((message) => {
              ...
              return (
                <motion.div
                  ...
                  className={`flex gap-2 sm:gap-3.5 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}
                >
                  {/* Avatar */}
                  <div
                    className={`w-7 h-7 sm:w-9 sm:h-9 rounded-xl sm:rounded-2xl flex-shrink-0 flex items-center justify-center ${
                      isUser
                  ...
                  >
                    {isUser ? (
                      <span className="text-[10px] sm:text-xs font-bold font-mono">ВИ</span>
                    ) : (
                  ...
                  {/* Message Bubble */}
                  <div
                    className={`max-w-[92%] sm:max-w-[80%] rounded-2xl p-3 sm:p-5 relative shadow-lg ${
                      isUser
          ...
          {/* Bottom Input Area */}
          <div className="relative z-10 p-3 sm:p-5 border-t border-white/[0.08] bg-[#090d18]/95 backdrop-blur-xl">
            ...
            <AnimatePresence>
              {isCodeInputVisible && (
                <motion.div
                  ...
                  className="mb-2.5 sm:mb-3 overflow-hidden"
                >
                  <div className="p-2.5 sm:p-3 bg-black/40 border border-rose-500/30 rounded-2xl relative">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-end gap-2">
              <div className="flex-1 bg-white/[0.04] border border-white/[0.1] focus-within:border-rose-500/60 focus-within:ring-2 focus-within:ring-rose-500/20 rounded-2xl p-2 sm:p-2.5 transition-all">
                <textarea
                  ref={inputRef}
                  value={inputValue}
                ...
                <div className="flex items-center justify-between pt-1 border-t border-white/[0.04] mt-1 text-[10px] sm:text-[11px] text-slate-400">
                  <button
                    type="button"
                    onClick={() => setIsCodeInputVisible(!isCodeInputVisible)}
                    className={`flex items-center gap-1 sm:gap-1.5 px-2 py-0.5 sm:py-1 rounded-lg text-[11px] sm:text-xs font-medium transition-colors ${
                      isCodeInputVisible
                  ...
                  >
                    ...
                    <span>{isCodeInputVisible ? 'Код додано' : '+ Код'}</span>
                  </button>
