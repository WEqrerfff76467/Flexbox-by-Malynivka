...
export const CheatSheetModal: React.FC<CheatSheetModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/70 backdrop-blur-xl animate-fadeIn">
      <div className="glass-panel w-full max-w-3xl rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[88vh]">
        {/* Header */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold text-white flex items-center gap-2">
                <span>Швидка Шпаргалка CSS Flexbox</span>
                <span className="text-[10px] bg-rose-500/15 text-rose-300 border border-rose-500/30 px-2.5 py-0.5 rounded-full font-mono">
                  Malynivka Edition
                </span>
              </h2>
              <p className="text-xs text-slate-300/80">Усі 12 властивостей та синтаксис в одному місці</p>
            </div>
          </div>
          <button
        ...
        <div className="p-4 border-b border-white/[0.08] bg-black/20">
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Пошук властивості (наприклад: justify, align-self, gap, grow)..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full bg-white/[0.04] border border-white/[0.08] rounded-full pl-11 pr-4 py-2.5 text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none focus:border-rose-500/60 focus:bg-white/[0.08] backdrop-blur-md transition-all"
            />
          </div>
        </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-3 flex-1">
          {filteredRules.map((rule, idx) => (
            <div
              key={idx}
              className="p-3.5 bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.06] hover:border-rose-500/40 rounded-2xl transition-all group backdrop-blur-md"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                    <code className="text-xs sm:text-sm font-bold text-rose-300 font-mono bg-rose-500/15 px-2.5 py-0.5 rounded-full border border-rose-500/20">
                      {rule.prop}
                    </code>
                    <span className="text-[10px] text-slate-300/80 bg-white/[0.06] border border-white/[0.06] px-2.5 py-0.5 rounded-full">
                      {rule.target}
                    </span>
                  </div>
                ...
                <button
                  ...
                  className="shrink-0 p-2 px-3 rounded-full bg-white/[0.04] hover:bg-rose-500 text-slate-300 hover:text-white border border-white/[0.08] transition-all text-xs flex items-center gap-1.5 active:scale-95"
                  title="Скопіювати рядок"
                >
                  {copiedText === rule.prop ? (
                    <>
                      <Check className="w-3.5 h-3.5 text-emerald-400" />
                      <span className="text-[10px] text-emerald-400 font-semibold">Скопійовано!</span>
                    </>
                  ) : (
          ...
          <div className="mt-4 p-4 rounded-2xl bg-rose-500/5 border border-rose-500/20 backdrop-blur-md">
            <h4 className="text-sm font-bold text-rose-300 flex items-center gap-1.5 mb-2">
              <Sparkles className="w-4 h-4 text-amber-300" />
              Золоте правило BEM у Flexbox:
            </h4>
            <pre className="text-xs font-mono text-slate-300 bg-black/40 p-3 rounded-xl border border-white/[0.06] overflow-x-auto">
...
.card-list__item--featured { align-self: flex-start; } /* Модифікатор */`}
        ...
        {/* Footer */}
        <div className="px-6 py-3 bg-white/[0.02] border-t border-white/[0.08] flex items-center justify-between text-xs text-slate-400">
          <span>Натисніть на іконку копіювання біля будь-якої властивості</span>
          <button
            onClick={onClose}
            className="px-5 py-1.5 rounded-full bg-rose-500 hover:bg-rose-600 text-white font-semibold transition-all shadow-[0_2px_10px_rgba(244,63,94,0.4)] active:scale-95"
          >
            Закрити
          </button>
        </div>
